...
<?php // Dieser Teil startet die Seiten, schreibt den Kopf, enth�lt die Funktionen, definiert die DB-Tabellen, speichert �bergebene Werte und l�dt die Variablen zur Umfrage
...
// Gemeinsame Funktionen zuladen
 ...
 include ('funktionen.php'); // Funktionen f�r die spezielle Anwendung laden
...
// Funktionen und Texte nur f�r dieses Projekt:
...
// Funktionen
 ...
 function weiter ($einzeln,$ue,$ressorts,$anforderungsliste,$fragen) { // zeigt das untere Menu oberhalb der Fu�leiste
	...
	} else { // Wahlausschuss-Seite  - ist das �berhaupt noch gefragt??? **********************
	
	if ($spielwiese) $erg = '&spielwiese=1';
	echo '<tr><td valign="middle" colspan="'.$spalten.'" class="rh">';
	//	echo '<span class="text3b"><br>Klicke auf das jeweilige Foto, um Details �ber den Kandidaten zu sehen!<br><br>';
	if ($ue) {
 ...
 function speichernmitbem ($name,$is0,$is8,$file) {  // Antworten und Bemerkungen ins Antwortfile schreiben
	...
	//$name.lfdNr ist auch der per POST �bergebene Wert; b.$name.lfdNr ist die zugeh�rige Bemerkung
	...
	$aendern = 0;
	$ein = 0; // Das ist der Z�hler, ob �berhaupt etwas �bergeben wurde
	$eintrag = 'UPDATE '.$file.' SET ';

	// Aktuelle Werte aus der Datenbank holen f�r Vergleich
	...
	for ($i=$is0;$i<=$is8;$i++){ // die lfdNrn durchgehen
		$f = $name.$i; // Feldbezeichnung in der Datenbank und bei der Daten�bergabe per POST der jeweilige Wert
		//echo "<br> f ist $f und ipost ist ".ipost($f);
		$wert = ipost($f); if ($wert > 5) $wert= 5; // Keine Werte �ber 5 m�glich
		if ($wert > 0) {
			$ein++; // Es wurde f�r diese Position etwas �bergeben
			$wert = ABS($wert*10000);
			} else {
		...
		// Nur speichern wenn Text ge�ndert wurde
		if (laenge($bem) > 1) {
			$ein++; // Es wurde ein Bemerkungstring mit L�nge > 1 �bergeben
			// Pr�fen ob der alte Text sich vom neuen unterscheidet
			...
			// Nur neue Bemerkung speichern wenn Text sich ge�ndert hat
			if ($bem != $alterText) {
...
// Authentifizierungs- und Anonymisierungsteil
 ...
 // Nur Ms erreichen diese Seiten, indem der Mensa-Server deren MNr �bergibt
 // Um die Anonymit�t zu erreichen, wird im Adressfile der MNr ein Key zugeordnet
 // Unter diesem Key werden im Antwortenfile die Antworten gespeichert
 // Wird der Key gel�scht (am Ende der Umfrage bzw. auf Anforderung des Nutzers), ist eine Zuordnung Key-MNr nicht mehr m�glich

 // Deshalb muss zu allen Seiten der Key �bergeben (durch GET oder POST) oder die MNr abgerufen werden
 // Was im Einzelnen geschieht, wenn kein Key �bergeben wird, wird auf den Einzelseiten geregelt


// Spezieller Teil: Kandidaten 
 // Die Identifizierung der Kandidaten in den spezifischen Tabellen kann �ber die M-Nummer erfolgen, weil hier keine Anonymisierung gewollt ist.
 // In der Antwortendatei zu Fragen/Antworten muss allerdings wie bei allen anderen Ms �ber den Schl�ssel identifiziert werden (Einheitlichkeit).
 // Deshalb d�rfen die Kandidaten ihren Schl�ssel nicht vorzeitig l�schen!




// Speicherteil: Wenn Daten �bergeben werden, werden sie hier gespeichert
 ...
 if (strpos($speichern,"endg")) { // individuellen Datensatz l�schen, aber als Basis f�r Meinungsbild erhalten
 ...
 if (($speichern) AND (!$verwerfen)) { // WahlOMat: Es wurden Antworten abgeschickt; die Antworten-Werte speichern
	...
	if (laenge($neust)+laenge($neufr)+laenge($neuth) > 20) {//Neue Frage abspeichern
		...
		dbspeichern($eintrag);
		// In der Antwortentabelle eine Spalte hinzuf�gen	
