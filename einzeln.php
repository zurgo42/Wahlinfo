<?php // Erste Seite Vorstandsfrageboten: Einzeldarstellung eines Kandidaten
 $dieseseite = "einzeln";
 $daten = 'textewahl'; // Hier stehen etliche f�r mehrere Seiten zu verwendende Texte
 $naechsteseite = "einzeln";
 $bildquelle = "../neuwahl/";

 include ("kopf.php"); // Seitenaufbau, Funktionen nur f�r diese Programmfamilie, Initialisierungen
...
// Den Kandidaten aufrufen
 ...
 $w0 = 140+($mobil>1)*100; $w1 = $w0+10; // Steuerung der Breite f�r Smartphone-Seiten
...
// Linke Spalte bzw. Wahlausschuss-Seite
 ...
 $erg = 0; // Hiermit werden die eingegebenen Elemente gez�hlt
...
// Vor Ende der Editierfrist sollen das nur die Kandidaten sehen und zwar nur deren eigene Seite
 ...
 if (!iget('spielwiese') AND ((time()<dzut($editieren_bis)) AND ($kand['mnummer'] <> $MNr))) { // nach Ende der Editierfrist
	...
	//Hinweis für eigene Seite entfernt - alle sehen gleiche Darstellung
 ...
 if (laenge($kand["team1"]) > 3) { // Die Mitkandidaten-Pr�ferenzen des Gezeigten zeigen
 ...
 // Alle diejenigen zeigen, die diesen Kandidaten pr�ferieren
 ...
 for ($i=1;$i<18;$i++) {$r = $r + ($kand["r$i"]>9999);} // Ressortw�nsche ausgef�llt
...
//	if ($a1) {
 ...
 if (($MNr == $kand['mnummer']) AND (time()<dzut($editieren_bis))) { // Angebot Bearbeiten w�hrend der Editierfrist
